...
namespace PikumaLessons
{
	Particle::Particle(const float x, const float y, const float mass, const int radius)
		: m_Position(x, y), m_Velocity(0.F), m_Acceleration(0.F), m_Mass(mass), m_InvMass(1.F/mass), m_Radius(radius)
	{
		std::cout << "Particle Constructor Called!" << '\n';
	}

	Particle::Particle(const Particle& other)
		: m_Position(other.m_Position), m_Velocity(other.m_Velocity), m_Acceleration(other.m_Acceleration), m_Mass(other.m_Mass), m_InvMass(other.m_InvMass), m_Radius(other.m_Radius)
	{
		std::cout << "Particle Copy Constructor Called!" << '\n';
	}

	Particle::Particle(Particle&& other) noexcept
	: m_Position(std::move(other.m_Position)), m_Velocity(std::move(other.m_Velocity)), m_Acceleration(std::move(other.m_Acceleration)), m_Mass(other.m_Mass), m_InvMass(other.m_InvMass), m_Radius(other.m_Radius)
	{
	...
	{
		// F = m*a into F/m = a formula
		m_Acceleration = m_ForcesSum * m_InvMass;
